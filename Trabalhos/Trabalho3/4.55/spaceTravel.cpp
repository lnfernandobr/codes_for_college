...
//
// g++ -o spaceTravel  spaceTravel.cpp -lglut -lGL -lGLU -lm
// Sumanta Guha.
///////////////////////////////////////////////////////////////////////////////////
...
Asteroid::Asteroid()
{
    ...
    color[1] = 0;
    color[2] = 0;
    /* Ex4.55 b) Ativa transparência, afim de fazer o asteróide brilhar */
    color[3] = 0;
}
...
/*
 * Ex4.55 b) Faz o asteroide dourado brilhar
 */
void glow(int value)
{
    unsigned char color = arrayAsteroids[ROWS / 2][COLUMNS / 2].getAlpha();
    if (color <= 150) {
        glowInc = 1;
    } else if (color >= 254) {
        glowInc = 0;
    ...
    if (!glowInc) {
        arrayAsteroids[ROWS / 2][COLUMNS / 2].setAlpha(color - 5);
    } else {
        arrayAsteroids[ROWS / 2][COLUMNS / 2].setAlpha(color + 5);
    }

    glutPostRedisplay();
    glutTimerFunc( 1, glow, 1 );
}
...
void setup(void)
{
    ...
    for (j=0; j<COLUMNS; j++)
        for (i=0; i<ROWS; i++)
            ...
            {
                /*
                 * Ex4.55 b) Faz o asteroide do centro maior e dourado
                 */
                if ((i == ROWS / 2) && (j == COLUMNS / 2))
                    arrayAsteroids[i][j] = Asteroid( 30.0*(-COLUMNS/2 + j), 0.0, -40.0 - 30.0*i, 8.0,
                                                     255, 215, 0);
        ...
        for (i=0; i<ROWS; i++)
            if (arrayAsteroids[i][j].getRadius() > 0 ) // If asteroid exists.
                if ( checkSpheresIntersection( x - 5 * sin( (PI/180.0) * a), 0.0,
                    ...
                    /*
                     * Ex4.55 d) Retorna 2 caso tenha encontrado o asteroide dourado
                     */
                    if ((i == ROWS / 2) && (j == COLUMNS / 2))
                        return 2;
                    else
...
void drawScene(void)
{
    ...
    if (isCollision == 1) writeBitmapString((void*)font, "Cannot - will crash!");
        /*
         * Ex4.55 d) Retorna 2 caso tenha encontrado o asteroide dourado
         */
    else if (isCollision == 2) writeBitmapString((void*)font, "You have found gold!");
    glPopMatrix();
    ...
    /*
     * Ex4.55 a) Coloca a câmera do visor da esquerda na ponta da nave
     */
    ...
    glPopMatrix();
    /*
     * Ex4.55 c) Desenha a antena da nave
     */
    glPushMatrix();
    glTranslatef(xVal, 16.0*5.0, zVal);
    glRotatef(angle, 0.0, 1.0, 0.0);
    glScalef(0.05, 32.0, 0.05);
    glutWireCube(5.0);
    glPopMatrix();
    // End left viewport.
    ...
    /*
     * Ex4.55 c) Câmera de dentro do asteróide dourado, apontando para a nave
     */
    gluLookAt(arrayAsteroids[ROWS / 2][COLUMNS / 2].getCenterX(),
              0.0,
              arrayAsteroids[ROWS / 2][COLUMNS / 2].getCenterZ(),
              xVal,
              0.0,
              zVal,
              0.0,
...
void specialKeyInput(int key, int x, int y)
{
    ...
    /*
     * Ex4.55 d) Seta a variável de colisão, e seta um número especial caso tenha chocado com o asteroide dourado
     */
    tmpCollision = asteroidCraftCollision(tempxVal, tempzVal, tempAngle);
    if (tmpCollision == 0)
    {
        isCollision = 0;
        xVal = tempxVal;
