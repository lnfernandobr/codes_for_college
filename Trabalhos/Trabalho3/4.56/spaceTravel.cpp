...
static float cameraX = 0, cameraZ = 0; // Co-ordinates of the camera.
...
void Asteroid::draw(int i) { // Function to draw asteroid.
    ...
    if (radius > 0.0) { // If asteroid exists.
        ...
        if(i % 2  == 0) {
            ...
            glColor3ubv(color);
            glutWireTorus((int)radius*1, (int)radius*1, 10, 10);
            glPopMatrix();
        }
...
void drawScene(void) {
    ...
    /**
     * 4.56 c) Fazer com que a camera gire para direita ou esqueda, sendo controlada pelo teclado.
     *
     * f1 move a camera para direita
     * f2 move a camera para esquerda
     **/

    // Locate the camera at the tip of the cone and pointing in the direction of the cone.
    gluLookAt(xVal - 10 * sin( (PI/180.0) * angle),
              0,
              zVal - (10 * cos( (PI/180.0) * angle) ) + cos( (PI/180.0) * cameraX ),
              xVal - (11 * sin( (PI/180.0) * angle) ) + sin( (PI/180.0) * cameraX ),
              0,
...
void animate(int x) {
    ...
    if(x < 3 )
        x % 2 == 0 ? increment(x + 2) : decrementAngle(x - 1);

    if(x >= 3)
        x % 2 == 0 ? incrementAngle(x - 1) : increment(x + 2);


    if(x == 5)
        x = 0;

   glutTimerFunc( 50, animate, x + 1);
   glutPostRedisplay();
...
//        cameraZ -= 1;
    ...
    // Angle correction.
    if (tempAngle > 360.0 || cameraX > 360.0)
        tempAngle -= 360.0;

    if (tempAngle < 0.0 || cameraX < 0.0)
        tempAngle += 360.0;
