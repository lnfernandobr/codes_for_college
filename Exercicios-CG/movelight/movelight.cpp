...
char *left_light, *right_light, *top_light;
char *ico_material, *teapot_material, *torus_material, *sphere_material;
...
void top_light_select(int value) {
    light_select(GL_LIGHT2, value, &top_light);
}
...
{
    int left_light_m, right_light_m, torus_m, teapot_m, shepre_m, ico_m, top_light_m;
...
#define MATERIAL_MENU_ENTRIES() \
    ...
    //top_light_select
    top_light_m = glutCreateMenu(top_light_select);
    LIGHT_MENU_ENTRIES();
    ...
    shepre_m=glutCreateMenu(shpere_select);
    MATERIAL_MENU_ENTRIES();
    ...
    glLightfv(GL_LIGHT2, GL_POSITION, top_light_position);
