   ...
   for(j = 0; j < q; j++) {
        ...
        glBegin(GL_TRIANGLE_STRIP);
         ...
         for(i = 0; i <= p; i++) {
            ...
            // glVertex3f( 
            //       A  * cos((i / p) * (2.0 * PI)) * cos((i / q) * (PI / 2.0)),
            //       B  * sin((i / p) * (2.0 * PI) ),
            //       C  * cos((i / p) * (2.0 * PI)) * sin((i / q) * (PI / 2.0))
            // );

            glVertex3f( 10 * cos( (float)(j+1)/q * PI/2.0 ) * cos( 2.0 * (float)i/p * PI ),
                  3 * sin( (float)(j+1)/q * PI/2.0 ),
                  8 * cos( (float)(j+1)/q * PI/2.0 ) * sin( 2.0 * (float)i/p * PI ) );


            glVertex3f( 10 * cos( (float)(j)/q * PI/2.0 ) * cos( 2.0 * (float)i/p * PI ),
                  3 * sin( (float)(j)/q * PI/2.0 ),
                  8 * cos( (float)(j)/q * PI/2.0 ) * sin( 2.0 * (float)i/p * PI ) );

            // glVertex3f( R * cos( (float)(j+2)/q * PI/2.0 ) * cos( 2.0 * (float)i/p * PI ),
            //       R * sin( (float)(j+2)/q * PI/2.0 ),
            //       R * cos( (float)(j+2)/q * PI/2.0 ) * sin( 2.0 * (float)i/p * PI ) );
